...
module.exports = {
    ...
    async execute(interaction) {
        ...
        for (const member of guildMembers.values()) {
            ...
            const lastdate = data.last_register_date;
        ...
        if (namelist.length === 0) {
            await interaction.editReply({ content: '登録が古いまたは未登録のユーザーはいませんでした。' });
            return;
        }

        const content = `登録が古いまたは未登録のユーザー: ${namelist.join(', ')}`;
        if (content.length > 1900) {
            const previewCount = 50;
            const preview = namelist.slice(0, previewCount).join(', ');
            await interaction.editReply({
                content: `登録が古いまたは未登録のユーザーは ${namelist.length} 人です。先頭 ${previewCount} 人: ${preview}`
            });
            return;
        }
